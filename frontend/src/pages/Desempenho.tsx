...
import { fetchJsonWithAuth, fetchWithAuth, getToken } from "../lib/fetchWithAuth";
...
export default function DesempenhoPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data: AulaItem[] = await fetchJsonWithAuth(`${API_URL}/aulas`);
        setAulas(data.map((d) => ({ id: d.id, titulo: d.titulo })));
      } catch (err: any) {
        console.error(err);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res: AulaDesempenho = await fetchJsonWithAuth(`${API_URL}/aulas/${selectedAulaId}/desempenho`);
        setDesempenho(res);
      } catch (err: any) {
        console.error(err);
  ...
  const chartData = useMemo(() => {
    ...
    return desempenho.questoes.map((q, i) => ({
      name: shortText(q.enunciado, 30) || `Q${i + 1}`,
      taxa_acerto: Number(q.taxa_acerto) || 0,
      total_respondentes: q.total_respondentes,
  ...
  function handleExportQuestionsCsv() {
    ...
    const rows = desempenho.questoes.map((q) => ({
      ...
      total_respondentes: q.total_respondentes,
      total_acertos: q.total_acertos,
      taxa_acerto: q.taxa_acerto,
      distribuicao: q.distribuicao_respostas ? JSON.stringify(q.distribuicao_respostas) : "",
    }));
    exportToCsv(`desempenho_aula_${desempenho.aula_id}_questoes.csv`, rows);
  }
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
            <div className="flex-1">
              ...
              <select
                ...
                onChange={(e) => setSelectedAulaId(e.target.value ? Number(e.target.value) : null)}
              >
            ...
            <div className="flex items-stretch gap-2">
              <button
                onClick={() => {
                  if (!selectedAulaId) return alert("Selecione uma aula.");
                  // refetch
                  setSelectedAulaId((s) => (s ? s : selectedAulaId));
                }}
                className="px-4 py-2 rounded bg-[#083D77] text-white hover:bg-[#062f63]"
              >
                Atualizar
              </button>
              <button onClick={handleExportQuestionsCsv} className="px-4 py-2 rounded border">
          ...
          <div className="mt-4 flex flex-col sm:flex-row gap-3">
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="bg-gray-50 p-4 rounded">
                ...
                <div className="text-2xl font-bold">{desempenho ? desempenho.media_pontuacao.toFixed(2) : "—"}</div>
              </div>
              <div className="bg-gray-50 p-4 rounded">
                <div className="text-sm text-gray-500">% Conclusão</div>
                <div className="text-2xl font-bold">{desempenho ? desempenho.percentual_conclusao.toFixed(2) + "%" : "—"}</div>
              </div>
            </div>
                ...
                <table className="min-w-full text-left">
                  ...
                  <tbody>
                    {desempenho.questoes.map((q, i) => (
                      <tr key={q.exercicio_id} className="border-t">
                        ...
                        <td className="px-3 py-2 align-top">{q.taxa_acerto?.toFixed?.(2) ?? q.taxa_acerto}%</td>
              ...
              <div className="mt-4">
                ...
                <div className="space-y-3">
                  {alunoDetail.responses.map((r) => (
                    <div key={r.exercicio_id} className="border rounded p-3 bg-gray-50">
                      ...
                      <div className="mt-1 text-sm">
                        ...
                        <span className="ml-2 text-sm text-gray-600">Pontuação: {r.pontuacao_obtida}</span>
                      </div>
                    </div>
                  ))}
                </div>
